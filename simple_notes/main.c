...
gint32 simple_notes_command_line (GApplication *application, GApplicationCommandLine *cmdline) {
    ...
    SimpleNotesNoteWindow *notesWindow = simple_notes_note_window_new(SIMPLE_NOTES_RESPONDER(stub), mediator);
    SimpleNotesFolderWindow *foldersWindow = simple_notes_folder_window_new(SIMPLE_NOTES_RESPONDER(notesWindow), mediator);
    SimpleNotesHelpWindow *help = simple_notes_help_window_new(SIMPLE_NOTES_RESPONDER(foldersWindow));
    SimpleNotesApplication *notesApplication = simple_notes_application_new(SIMPLE_NOTES_RESPONDER(help));
...
//        gchar buff[] = {'n','o','t','e',' ','-','-','a','l','l','\0'};
        gssize count = g_input_stream_read(stdStream, buff, symbolsMax, NULL, &error);
        ...
        if (success) {
            ...
            SimpleNotesEvent *event = simple_notes_event_new(buff, count, &error);
            success = event != NULL;
            if (success) {
                simple_notes_responder_handle_event(SIMPLE_NOTES_RESPONDER(notesApplication), event);
                g_object_unref(event);
            }
        }
        if (!success) {
            simple_notes_responder_handle_error(SIMPLE_NOTES_RESPONDER(notesApplication), error);
        }
        g_clear_error(&error);
    }
...
gint32 main (int argc, char **argv)
{
    ...
    app = g_application_new("org.simple-notes.example", G_APPLICATION_HANDLES_COMMAND_LINE);
    g_signal_connect(app, "command-line", G_CALLBACK(simple_notes_command_line), NULL);
