...
static void
simple_notes_sqlite_controller_fixture_set_up (SimpleNotesSQLiteControllerFixture *fixture, gconstpointer user_data) {
    simple_notes_delete_test_db();
}

static void
simple_notes_sqlite_controller_fixture_tear_down (SimpleNotesSQLiteControllerFixture *fixture, gconstpointer user_data) {
    
}

static void simple_notes_sqlite_controller_check_other_folder_fields (SimpleNotesFolder *folder) {
    g_assert_cmpuint(simple_notes_object_get_id(SIMPLE_NOTES_OBJECT(folder)), >, 0);
    g_assert_cmpint(simple_notes_folder_get_count(folder), ==, 0);
    g_assert_false(simple_notes_object_get_selected(SIMPLE_NOTES_OBJECT(folder)));
}

static void simple_notes_sqlite_controller_check_title (GList *item, gchar value[]) {
...
static void simple_notes_sqlite_controller_check_folder (GList *item, gchar title[]) {
    simple_notes_sqlite_controller_check_other_folder_fields(item->data);
    simple_notes_sqlite_controller_check_title(item, title);
}

static void simple_notes_sqlite_controller_check_folders (GList *folders, gchar *titleValues[]) {
    glong i = 0;
    while (folders) {
...
static void
test_simple_notes_sqlite_controller_folders (SimpleNotesSQLiteControllerFixture *fixture, gconstpointer user_data) {
    ...
    g_byte_array_append(newTitle, (const guint8 *)newString, (guint)strlen(newString) + 2);
    simple_notes_folder_assign_count(updatedFolder, newCount);
    simple_notes_folder_copy_title(updatedFolder, newTitle);
    simple_notes_object_assign_selected(SIMPLE_NOTES_OBJECT(updatedFolder), TRUE);
    ...
    guint64 deletedID = simple_notes_object_get_id(SIMPLE_NOTES_OBJECT(newFolder));
    GList *foldersAfterDelete = simple_notes_sqlite_controller_copy_folders();
    GList *item = foldersAfterDelete;
    while (item) {
        guint64 itemID = simple_notes_object_get_id(SIMPLE_NOTES_OBJECT(item->data));
...
static void
test_simple_notes_sqlite_controller_notes (SimpleNotesSQLiteControllerFixture *fixture, gconstpointer user_data) {

    guint64 folderID = 1;
    gchar *content = "New content\0";
    time_t lastEdited = time(NULL);
    simple_notes_sqlite_controller_insert_note((guint8 *)content, folderID, lastEdited);
    ...
    g_byte_array_append(newContent, (const guint8 *)newString, (guint)strlen(newString) + 2);
