...
(SimpleNotesFoldersModelFixture *fixture, gconstpointer user_data)
{
...
static void
test_simple_notes_folders_model_all (SimpleNotesFoldersModelFixture *fixture,
                                     gconstpointer user_data)
{
  guint count = 0;
  SimpleNotesFoldersModel *model
    = simple_notes_mediator_get_folders_model (fixture->_model);
  SimpleNotesLightFolder **empty
    = simple_notes_folders_model_copy_folders (model, &count);
  g_assert_cmpint (count, ==, 0);
  g_free (empty);

  gchar *const name = "Новая Папка\0";
  simple_notes_folders_model_insert_folder (model, (guint8 *)name);
  simple_notes_folders_model_insert_folder (model, NULL);
  SimpleNotesLightFolder **newFolders
    = simple_notes_folders_model_copy_folders (model, &count);
  ...
  GByteArray *nullTitle
    = simple_notes_light_folder_get_copy_title (newFolders[1]);
  g_assert_cmpstr ((gchar *)nullTitle->data, ==, NULL);
  g_byte_array_unref (nullTitle);

  simple_notes_folders_model_delete_folder
    (model,
     simple_notes_light_folder_get_id(newFolders[1]));
  SimpleNotesLightFolder **afterDelete
    = simple_notes_folders_model_copy_folders (model, &count);
  g_assert_cmpuint (count, ==, 1);

  guint64 identifier = simple_notes_light_folder_get_id (afterDelete[0]);
  g_assert_cmpuint (identifier,
                    ==,
                    simple_notes_light_folder_get_id (newFolders[0]));
  ...
  simple_notes_folders_model_select_folder (model, identifier);
  SimpleNotesLightFolder **selectedFolder
    = simple_notes_folders_model_copy_folders (model, &count);
  guint64 selectedID = simple_notes_light_folder_get_id (selectedFolder[0]);
  g_assert_cmpuint (count, ==, 1);
  g_assert_cmpuint (identifier, ==, selectedID);
  g_assert_cmpint (simple_notes_light_folder_get_selected (selectedFolder[0]),
                   ==,
                   TRUE);
  ...
  simple_notes_folders_model_change_folder_title
    (simple_notes_mediator_get_folders_model (fixture->_model),
     selectedID,
     (guint8 *)newName,
     (guint)strlen (newName) + 2);
  SimpleNotesLightFolder **newNameFolder
    = simple_notes_folders_model_copy_folders (model, &count);
  GByteArray *newTitle
    = simple_notes_light_folder_get_copy_title (newNameFolder[0]);
  g_assert_cmpuint (count, ==, 1);
